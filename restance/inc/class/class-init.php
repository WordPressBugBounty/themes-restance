...
class Init {
	...
	public function change_stylesheet_directory() {
		return RESTANCE_DIR . 'gutenverse-files';
	}
	...
	public function template_path( $template_file, $theme_slug, $template_slug ) {
		switch ( $template_slug ) {
			...
			case 'news':
				return $this->change_stylesheet_directory() . '/templates/news.html';
			case 'page':
				return $this->change_stylesheet_directory() . '/templates/page.html';
			case 'property-detail':
				return $this->change_stylesheet_directory() . '/templates/property-detail.html';
			case 'property-list':
				return $this->change_stylesheet_directory() . '/templates/property-list.html';
			case 'search':
			...
			case 'footer-2':
				return $this->change_stylesheet_directory() . '/parts/footer-2.html';
		}
	...
	public function notice_closed() {
		if ( isset( $_POST['nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'restance_admin_notice' ) ) {
	...
	public function notice_install_plugin() {
		...
		if ( isset( $screen->parent_file ) && 'themes.php' === $screen->parent_file && 'appearance_page_restance-dashboard' === $screen->id ) {
			return;
		}
		...
		$button_text = __( 'Install Required Plugins', 'restance' );
		...
		<style>
			.install-gutenverse-plugin-notice {
				...
				background: url( <?php echo esc_url( RESTANCE_URI . '/assets/img/background-banner.png' ); ?> );
		...
		jQuery( function( $ ) {
			$( 'div.notice.install-gutenverse-plugin-notice' ).on( 'click', 'button.notice-dismiss', function( event ) {
				...
				$.post( ajaxurl, {
					...
					nonce: '<?php echo esc_html( wp_create_nonce( 'restance_admin_notice' ) ); ?>',
				} );
			} );
			...
			<div class="gutenverse-notice-inner">
				<div class="gutenverse-notice-content">
					<div class="gutenverse-notice-text">
						<h3><?php esc_html_e( 'Take Your Website To New Height with', 'restance' ); ?> <span>Gutenverse!</span></h3> 
						<p><?php esc_html_e( 'Restance theme work best with Gutenverse plugin. By installing Gutenverse plugin you may access Restance templates built with Gutenverse and get access to more than 40 free blocks, hundred free Layout and Section.', 'restance' ); ?></p>
					...
					<div class="gutenverse-notice-image">
						<img src="<?php echo esc_url( RESTANCE_URI . '/assets/img/banner-install-gutenverse-2.png' ); ?>"/>
					</div>
				</div>
			</div>
		</div>
	...
	public function backend_scripts() {
		...
		if ( $screen->id === 'appearance_page_restance-dashboard' ) {
			...
			wp_localize_script( 'restance-dashboard', 'GutenverseThemeConfig', $this->theme_config() );
		}
	}
	...
	public function theme_config() {
		return array(
			'demo'    => esc_url( 'https:/gutenverse.com/demo?name=restance' ),
			'pages'   => array(
